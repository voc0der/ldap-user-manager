...
// --- App/site display name (to match other emails from the app) ---
...
// --- MAILER (PHPMailer if available; consistent "X User Manager <addr>" From) ---
function mtls_send_mail($to, $subject, $body, $fallbackFrom) {
  ...
  if (preg_match('/^\s*([^<]+?)\s*<\s*([^>]+)\s*>\s*$/', $fromAddr, $m)) {
    $fromAddr = trim($m[2]); // normalize to plain addr; we'll set the name below
  }
...
elseif ($action === 'verify_code') {
  ...
  // UI will poll token_info for expires_days and p12_pass (host stager writes them)
  json_ok(array('token'=>$token, 'expires_days'=>null));
}
elseif ($action === 'token_info') {
  ...
  $pw = isset($rec2['p12_pass']) && is_string($rec2['p12_pass']) && $rec2['p12_pass'] !== ''
    ? $rec2['p12_pass'] : null;
